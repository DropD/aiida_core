...
class ComputerParam(click.ParamType):
    ...
    def __init__(self, convert=True, pass_pk=False):
    ...
    @aiida_dbenv
    def complete(self, ctx=None, incomplete=''):  # pylint: disable=unused-argument,no-self-use
        """
        ...
        with cli_spinner():
            ...
            query_builder.append(type='computer', project=['name'])
            names = [i for j in query_builder.iterall() for i in j if startswith(i, incomplete)]
            results += [(i, None) for i in names]
            query_builder = QueryBuilder()
            query_builder.append(type='computer', project=['id', 'name'])
            results += [(str(i[0]), i[1]) for i in query_builder.iterall() if startswith(str(i[0]), incomplete)]
            return results
