...
def import_qeinput(fname):
    ...
    def get_pos(pos, coord_type, alat, cell):
        ...
        if coord_type == pos_types[0]:
            return np.array(pos) * alat * bohr
        if coord_type == pos_types[1]:
            return np.array(pos) * alat
    ...
    while line:
        ...
        if nat_search.search(line):
            nat = int(nat_search.search(line).group(3))
        ...
        if a_search.search(line):
            a = float(a_search.search(line).group(3))
        if b_search.search(line):
            b = float(b_search.search(line).group(3))
        if c_search.search(line):
            c = float(c_search.search(line).group(3))
